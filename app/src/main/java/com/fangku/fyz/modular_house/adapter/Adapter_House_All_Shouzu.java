...
import com.fangku.fyz.modular_house.all_room.House_All_DCost;
 ...
 * Time: 15:50
 */
public class Adapter_House_All_Shouzu extends BaseListAdapter<Bean_House_All_Contract.DataEntity.ZjmxEntity> {
    public Adapter_House_All_Shouzu(Activity context, List<Bean_House_All_Contract.DataEntity.ZjmxEntity> list) {
        super(context, list);
    }
    ...
    public View bindView(int position, View convertView, ViewGroup parent) {
        ...
        mTextMoney.setText(String.valueOf(list.get(position).getTotalCost()));
        ...
        if ("2".equals(list.get(position).getChargeType())) {
            ...
            if (list.get(position).getCostName().equals("水表")) {
                mTextMeter.setText(list.get(position).getTotalCost() + "元/立方");
            } else {
                mTextMeter.setText(list.get(position).getTotalCost() + "元/度");
            }
        ...
        mLl.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                House_All_DCost.launch(mContext, String.valueOf(list.get(position).getTotalCost()), String.valueOf(list.get(position).getCostPrice()), position);
            }
        });
